...
- relation_subtype:
  ...
  A more specific relation_subtype of the relation, derived from the
  `<relation_subtype>` child elements of the `<relation>` node (e.g.,
  `'activation'`, `'inhibition'`, `'expression'`, `'compound'`). If no
  relation_subtype is defined, this will be `NA`.

- relation_value:

  A categorical value associated with the relation_subtype, which
  encodes the apearence of the arrow (from the `value` attribute of
  `<relation_subtype>`). If not present, this will be `NA`.
