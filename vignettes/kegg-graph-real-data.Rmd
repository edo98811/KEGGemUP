...
These vignette assumes you have already performed a differential expression analysis and have the results available as a `data.frame` (whihc you can easily obtain from edgeR or limma by running `as.data.frame()` on the table of the results for each object). To reproduce this situation we will now build a examplary differential expression results using the macrophage dataset from the macrophage package.
...
graph <- map_results_to_graph(graph, de_results_list, return_type = "visNetwork")
graph
```
