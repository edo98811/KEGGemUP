...
The workflow is as follows: first we build the graph from the KEGG pathway ID, then we map the differential expression results to the nodes of the graph, and at the end we plot the graph using `plot_kegg_visNetwork()`, which creates an interactive visualization of the graph using visNetwork.
...
graph_visnetwork <- plot_kegg_visNetwork(graph)
graph_visnetwork
```
